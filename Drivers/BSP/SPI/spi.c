...
void spi2_init(void)
{
    ...
    g_spi2_handler.Instance = SPI2;                                /* SPI2 */
    ...
    spi2_set_speed(SPI_SPEED_2);//传输速度设置为18MHZ(5~20都可以),计算式为：36/2^(speed+1),36为单片机时钟总线的频率
    __HAL_SPI_ENABLE(&g_spi2_handler); /* 使能SPI2 */
    spi2_write_byte(0Xff); /* 启动传输, 实际上就是产生8个时钟脉冲, 达到清空DR的作用, 非必需 */
}
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    {
        __HAL_RCC_GPIOB_CLK_ENABLE();  /* SPI2_SCK/SPI2_MOSI脚时钟使能 */

        /* SCK引脚模式设置(复用输出) ,作为TFT的时钟*/
        gpio_init_struct.Pin = GPIO_PIN_13;
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;//复用推挽输出，复用时这个引脚作为SPI2_SCK
        gpio_init_struct.Pull = GPIO_PULLUP;
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
        HAL_GPIO_Init(GPIOB, &gpio_init_struct);

        /* MOSI引脚模式设置(复用输出) */
        gpio_init_struct.Pin = GPIO_PIN_15;//复用时这个引脚作为SPI2_MOSI
        HAL_GPIO_Init(GPIOB, &gpio_init_struct);
    }
}
...
/**
 * @brief       SPI2写一个字节数据
 * @param       data  : 要写的数据(1字节)
 * @retval      无
 */
void spi2_write_byte(uint8_t data)
{
    while((SPI2->SR & SPI_SR_TXE)==0){}; //等待发送缓冲区为空
    SPI2->DR = data;
    //333.33ns
    __NOP();__NOP();__NOP();
    __NOP();__NOP();__NOP();
