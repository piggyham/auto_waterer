...
RTC_HandleTypeDef g_rtc_handle; //RTC句斌
_calendar_obj calendar;         /* 时钟结构体 */
uint16_t bkpflag = 0; /* 备份域标志,用来判断RTC备份域有没有设置过 */
_Bool flag_alarm = 0; //闹钟标志
...
uint8_t rtc_init(void)
{
    ...
    if (HAL_RTC_Init(&g_rtc_handle) != HAL_OK)//判断RTC有没有初始化成功
    {
        return 1;
    }
    if ((bkpflag != 0X5050) && (bkpflag != 0x5051))         /* 之前未初始化过，重新配置 */
    {
        rtc_set_time(2024, 9, 25, 11, 47, 35);              /* 设置时间 */
    }    
    //__HAL_RTC_ALARM_ENABLE_IT(&g_rtc_handle, RTC_IT_SEC); /* 允许秒中断 */
    //rtc_set_time(2025, 6, 14, 23, 8, 00);
    //rtc_set_alarm(2025, 6, 14, 23, 8, 30);
    __HAL_RTC_ALARM_DISABLE_IT(&g_rtc_handle, RTC_IT_SEC); /* 关闭秒中断 */
    __HAL_RTC_ALARM_DISABLE_IT(&g_rtc_handle, RTC_IT_ALRA); /* 关闭闹钟中断 */
...
void RTC_IRQHandler(void)
{
    ...
    {
        __HAL_RTC_ALARM_CLEAR_FLAG(&g_rtc_handle, RTC_FLAG_SEC);            /* 清除秒中断标志 */
    }
