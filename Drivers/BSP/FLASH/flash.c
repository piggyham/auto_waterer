...
void stmflash_write(uint32_t waddr, uint16_t *pbuf, uint16_t length)
{
    ...
    secpos = offaddr / STM32_SECTOR_SIZE;       /* 扇区地址(页号)  0~255 for STM32F103ZET6 */
    ...
    while (1)
    {
        stmflash_read(waddr, g_flashbuf, STM32_SECTOR_SIZE / 2); /* 读出整个扇区的内容 */
        /* 注：secpos * STM32_SECTOR_SIZE + STM32_FLASH_BASE == waddr */
        ...
        { 
            ...
            flash_eraseop.Banks = FLASH_BANK_1; /* 擦除的Bank编号 */
            flash_eraseop.NbPages = 1;  /* 要擦除的页面数 */
            flash_eraseop.PageAddress = waddr;  /* 要擦除的扇区 */
            ...
            stmflash_write_nocheck(waddr, g_flashbuf, STM32_SECTOR_SIZE / 2); /* 写入整个扇区 */
        }
        else
        {
